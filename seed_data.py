...
def get_seed_data():
    ...
    length_key = UnitType(type='length',
        units = ['meter', 'foot', 'inch', 'yard', 'mile', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec']).put()
    meter_key = Unit(name='meter', type='length', abbreviation='m',
        convert_units = ['meter', 'mile', 'foot', 'inch', 'yard', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.0', '0.000621371', '3.28084', '39.37008', '1.09361', '0.00497096', '1e+10', '6.68459e-12', '1.057e-16', '3.24078e-17']).put()
    foot_key = Unit(name='foot', type='length', abbreviation='ft',
        convert_units = ['foot', 'mile', 'meter', 'inch', 'yard', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.0', '0.000189394', '0.3048', '12.0', '0.333333', '0.00151515', '3.048e+9', '2.03746e-12', '3.22174e-17', '9.8779e-18']).put()
    inch_key = Unit(name='inch', type='length', abbreviation='in',
        convert_units = ['inch', 'mile', 'meter', 'foot', 'yard', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.0', '1.5783e-5', '0.0254', '0.0833333', '0.0277778', '0.000126262', '2.54e+8', '1.69789e-13', '2.68478e-18', '8.23158e-19']).put()
    yard_key = Unit(name='yard', type='length', abbreviation='yd',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.0', '0.000568182', '0.9144', '36.0', '3.0', '0.00454545', '9.144e+9', '6.11239e-12', '9.66522e-17', '2.96337e-17']).put()
    mile_key = Unit(name='mile', type='length', abbreviation='mi',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1760.0', '1.0', '1609.34', '63360', '5280', '7.99998', '1.609e+13', '1.07578e-8', '1.70108e-13', '5.21553e-14']).put()
    furlong_key = Unit(name='furlong', type='length', abbreviation='fur',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['220', '0.125', '201.168', '7920.02', '660.001', '1.0', '2.012e+12', '1.34473e-9', '2.12635e-14', '6.51942e-15']).put()
    angstrom_key = Unit(name='angstrom', type='length', abbreviation='&#8491;',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.09361e-10', '6.2137e-14', '1e-10', '3.937e-9', '3.2808e-10', '4.97096e-13', '1.0', '6.68459e-22', '1.057e-26', '3.24078e-27']).put()
    astronomical_unit_key = Unit(name='astronomical-unit', type='length', abbreviation='AU',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.636e+11', '9.296e+7', '1.496e+11', '5.89e+12', '4.908e+11', '7.436e+8', '1.496e+21', '1.0', '1.58125e-5', '4.84814e-6']).put()
    light_year_key = Unit(name='light-year', type='length', abbreviation='ly',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['1.035e+16', '5.879e+12', '9.461e+15', '3.725e+17', '3.104e+16', '4.703e+13', '9.461e+25', '63241.1', '1.0', '0.306601']).put()
    parsec_key = Unit(name='parsec', type='length', abbreviation='pc',
        convert_units = ['yard', 'mile', 'meter', 'inch', 'foot', 'furlong', 'angstrom', 'astronomical-unit', 'light-year', 'parsec'],
        convert_values = ['3.375e+16', '1.917e+13', '3.086e+16', '1.215e+18', '1.012e+17', '1.534e+14', '3.086e+26', '206265', '3.26156', '1.0']).put()

    time_key = UnitType(type='time',
        units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year']).put()
    second_key = Unit(name='second', type='time', abbreviation='s',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['1.0', '0.0166667', '0.000277778', '1.1574e-5', '1.6534e-6', '8.2672e-7', '3.8052e-7', '3.171e-8']).put()
    minute_key = Unit(name='minute', type='time', abbreviation='min',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['60.0', '1.0', '0.0166667', '0.000694444', '9.9206e-5', '4.96032e-5', '2.2831e-5', '1.9026e-6']).put()
    hour_key = Unit(name='hour', type='time', abbreviation='hr',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['3600.0', '60.0', '1.0', '0.0416667', '0.00595238', '0.00297619', '0.00136986', '0.000114155']).put()
    day_key = Unit(name='day', type='time', abbreviation='d',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['86400.0', '1440.0', '24.0', '1.0', '0.142857', '0.0714286', '0.0328767', '0.00273973']).put()
    week_key = Unit(name='week', type='time', abbreviation='wk',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['604800.0', '10080.0', '168.0', '7.0', '1.0', '0.5', '0.230137', '0.0191781']).put()
    fortnight_key = Unit(name='fortnight', type='time', abbreviation='ftn',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['1.21e+6', '20160.0', '336.0', '14.0', '2.0', '1.0', '0.460273', '0.0383562']).put()
    month_key = Unit(name='month', type='time', abbreviation='m',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['2.628e+6', '43800', '730.001', '30.4167', '4.34524', '2.17262', '1.0', '0.0833334']).put()
    year_key = Unit(name='year', type='time', abbreviation='yr',
        convert_units = ['second', 'minute', 'hour', 'day', 'week', 'fortnight', 'month', 'year'],
        convert_values = ['3.154e+7', '525600', '8760', '365', '52.1429', '26.0714', '12', '1.0']).put()

    mass_key = UnitType(type='mass',
        units = ['gram', 'atomic-mass-unit', 'slug', 'solar-mass']).put()
    gram_key = Unit(name='gram', type='mass', abbreviation='g',
        convert_units = ['gram', 'atomic-mass-unit', 'slug', 'solar-mass'],
        convert_values = ['1.0', '6.022e+23', '6.85218e-5', '5e-34']).put()
    atomic_mass_unit_key = Unit(name='atomic-mass-unit', type='mass', abbreviation='amu',
        convert_units = ['gram', 'atomic-mass-unit', 'slug', 'solar-mass'],
        convert_values = ['1.66054e-24', '1.0', '1.13783e-28', '8.3027e-58']).put()
    slug_key = Unit(name='slug', type='mass', abbreviation='sl',
        convert_units = ['gram', 'atomic-mass-unit', 'slug', 'solar-mass'],
        convert_values = ['14593.9', '8.789e+27', '1.0', '7.296951468602e-30']).put()
    solar_mass_key = Unit(name='solar-mass', type='mass', abbreviation='M<sub>&#9737;</sub>',
        convert_units = ['gram', 'atomic-mass-unit', 'slug', 'solar-mass'],
        convert_values = ['2e+33', '1.204427330335e+57', '1.370435317136e+29', '1.0']).put()
    ...
    energy_key = UnitType(type='energy',
        units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour']).put()
    joule_key = Unit(name='joule', type='energy', abbreviation='J',
        convert_units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour'],
        convert_values = ['1', '0.239006', '0.000947817', '6.242e+18', '2.7778e-7']).put()
    calorie_key = Unit(name='calorie', type='energy', abbreviation='cal',
        convert_units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour'],
        convert_values = ['4.184', '1', '0.00396567', '2.611e+19', '1.1622e-6']).put()
    british_thermal_unit_key = Unit(name='british-thermal-unit', type='energy', abbreviation='BTU',
        convert_units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour'],
        convert_values = ['1055.06', '252.164', '1', '6.585e+21', '0.000293071']).put()
    electron_volt_key = Unit(name='electron-volt', type='energy', abbreviation='eV',
        convert_units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour'],
        convert_values = ['1.6022e-19', '3.8293e-20', '1.5186e-22', '1', '4.4505e-26']).put()
    kilowatt_hour_key = Unit(name='kilowatt-hour', type='energy', abbreviation='kWh',
        convert_units = ['joule', 'calorie', 'british-thermal-unit', 'electron-volt', 'kilowatt-hour'],
        convert_values = ['3.6e+6', '860421', '3412.14', '2.247e+25', '1']).put()
